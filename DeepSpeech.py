...
def calculate_mean_edit_distance_and_loss(model_feeder, tower, dropout, is_training, reuse):
    ...
    decoded, _ = decode_with_lm(logits, out_lengths, merge_repeated=False, beam_width=FLAGS.beam_width)
...
def create_inference_graph(batch_size=None, use_new_decoder=False):
    ...
    # Calculate the logits of the batch using BiRNN
    logits, out_lengths = BiRNN(input_tensor, tf.to_int64(seq_length) if FLAGS.use_seq_length else None, no_dropout)

    # Beam search decode the batch
    decoder = decode_with_lm if use_new_decoder else tf.nn.ctc_beam_search_decoder

    decoded, _ = decoder(logits, out_lengths, merge_repeated=False, beam_width=FLAGS.beam_width)
